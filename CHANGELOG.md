...
- **The query string of a URL no longer reaches `log_runs` on a network error.** `_safe_url` stripped it from the messages the library writes — and `docs/security.md` stated the guarantee without qualification — but a `ConnectionError` is raised by urllib3 *before* the library regains control, and copies the requested URL into its own message: `Max retries exceeded with url: /items?api_key=SUPERSECRET&page=1`. That message became `RunResult.error_message` and was persisted into a Delta table readable by everyone with lakehouse access, which is precisely the audience `_safe_url` exists to keep it from. Every `requests` exception raised by a call is now rebuilt with its query redacted, looked for in the URL requests actually prepared, in the URL rebuilt from `params=` (an early failure carries no prepared request), and in the URL requested. The exception **type** is preserved, because the type is what decides whether the request is replayed — turning a `ConnectionError` into an `AuthError` would fail a run on a transient network incident. `token_url` now goes through the same stripping in the two auth messages that copied it verbatim.
- **Auth headers no longer follow a redirect off the original origin.** `requests` strips only the literal `Authorization` header when a redirect changes host; an `api_key_header` (default `X-API-Key`), or any `bearer_token` carried by a custom `header_name`, was re-sent to wherever a `302` pointed. `docs/security.md` scoped this risk to `next_link` on the grounds that the other strategies "only ever call `base_url`" — true of the URLs the library builds, irrelevant to where the requests end up, since it is the server and not the configuration that decides to redirect. The session now drops every header it set for authentication as soon as the redirect leaves the host, or downgrades `https` to `http`. Headers from `config['headers']` are untouched: validation already requires them to be literal, non-sensitive strings.
- **An API that caps the page size below the requested `limit` no longer truncates the load.** `paginate_offset` ended on the first page shorter than `limit`, which is right when the API honours the size it was asked for and silently wrong when it does not: `limit: 1000` against an API capping pages at 100 read **one page, 100 rows**, and the run was marked `success` on a source holding millions. The two cases are indistinguishable from a single page — a 100-row cap and a 100-row source return exactly the same thing — so the library now spends one more request to separate them, and only while the doubt exists: a full page already seen proves the size is honoured, which is the common case and costs nothing extra. When the cap is real, pagination continues at the size actually served and the run reports it in `RunResult.warnings` rather than failing; the data is complete, only the config is optimistic. The same rule now covers `keyset` and `page`. `paginate_offset` also advanced by `limit` rather than by the rows received, so a capped API made it skip the difference on every page; it advances by what was served.
- **A column frozen to text by an all-null first batch is no longer silent.** A column with no non-null value in its first batch is typed text for want of anything better — that rule stands. What did not hold was the consequence: the type went into `known_types`, later batches conformed, the values were converted, the Array was built without error, and `fallbacks` came back empty. A batch of `{"amount": None}` followed by a batch of `10.5` and `20` wrote `'10.5'` and `'20'` with nothing reported, and the text type was then frozen in the Delta table. This is the silent degradation the type inference was rewritten to remove, taking the one path where the fallback succeeds. The type is deliberately **not** recomputed — retyping the column would break the commit, and every later run with it, since the table already carries the text type. Reporting it is what was missing. Batches still all null say nothing, a genuinely textual column says nothing, and an int-to-float widening says nothing.
- **The repeated-page guard no longer fails legitimate runs.** It compared `repr(record)[:200]` for the first and last record of a page, so two genuinely different pages of equal length collided as soon as a verbose field preceded the identifier in the JSON key order — a record whose `description` holds 300 characters truncates identically whatever its `id`. The run then died on "the source is not advancing". The fingerprint is now a blake2b digest over the full repr of the first and last record. Still first and last rather than the whole page: what the guard looks for is an API re-serving the same page, and it re-serves it whole.
- **The OAuth1 `realm` is escaped before it reaches the header.** It travels in a quoted-string and stays out of the signature (RFC 5849 §3.5.1), but was interpolated raw: a double quote in an account identifier closed the string early and the server read a truncated realm. It is escaped per RFC 9110 §5.6.4, and a realm carrying a control character is refused at construction with a message naming the key, rather than by urllib3 with one that does not.
- **The session is closed when the run ends.** `_build_fetch_page` opened a `requests.Session` and nothing closed it, so a notebook iterating over sources left one connection pool per run alive until the garbage collector got to it. It is now closed in a `finally` covering the whole run.

### Changed

- **Numeric configuration values are validated, not just their key names.** `retry` was checked for unknown keys and nothing else, and `timeout_seconds` was not checked at all: a `"timeout_seconds": null` passed validation, reached `requests` as `timeout=None`, and parked the notebook forever — with no timeout, so none of the retries that should have followed ever happened. One helper now covers `batch_size`, `timeout_seconds`, `auth.timeout_seconds`, the three `retry` keys, and the pagination bounds `limit`, `page_size` and `start_page` alongside `max_pages` and `max_rows`. Zero stays valid where it means something — `max_retry_after_seconds` disables the server-dictated wait, `start_page` is right for an API indexed from 0 — and is refused everywhere else, as are booleans, which are `int` in Python and would silently become 1. Existing messages are unchanged.
- **`records_to_table` is about four times faster.** Two independent costs, both in the wrong dimension. Column names were gathered with a membership test over a list, scanning it once per key of every record: on a 50 000-row batch that loop alone took 0.13 s at 20 columns, 0.90 s at 60 and 3.05 s at 120, before any conversion started. And the per-value normalization compared the Arrow type against `string()` then `float64()` for **every value** — comparisons that cross the Python/Rust boundary at 0.65 µs each, two per value, about 11 of the 13 seconds of a 120-column batch spent answering the same two questions six million times. A `seen` set beside the list gives order and lookup both; `_normalizer` answers the type questions once per column and returns the closure that does the work. Measured end to end on 50 000-row batches: 11.7 s to 2.5 s at 120 columns, 5.9 s to 1.4 s at 60, 1.3 s to 0.3 s at 20. Column order and results are unchanged.

### Documentation

- **`docs/security.md` no longer claims properties the code stopped having.** "Writes are append-only Delta commits; the library never deletes or overwrites tables" has been false since the `write` block landed — `"mode": "overwrite"` empties the target table — and the document elsewhere states that anyone able to edit the configuration can redirect a token to a host they control. A reader taking the append-only line at face value would conclude that the same person could not destroy data. They can; the section says so, and names Delta time travel as the recovery path. "Auth failures (401/403) fail immediately and are never retried" contradicted the token renewal the library performs on a 401, now stated with its bound: one renewal, one replay, per page. The `next_link` warning gains the distinction it was missing — building a URL is not choosing a destination — and the watermark's SQL quoting is recorded as verified against DataFusion rather than assumed.
- **`docs/configuration.md`** documents the confirming request on a short first page, and the all-null column typed to text.
...
- **`incremental.normalize`, to reshape the watermark before it is sent.** The watermark makes a round trip — read from a record, sent back as a filter — and an API is free to disagree with itself about the format between the two. One that returns `2026-08-25T14:57:44.000+02:00` but only accepts `...Z` in its filter could not be loaded incrementally at all: the value was reinjected verbatim, and nothing existed to convert it, so the choice was to reimplement the whole watermark by hand in the notebook. `"normalize": "utc_iso"` converts it to UTC, `initial_value` included, so the first run and every run after it send the same shape; `none` remains the default and the previous behaviour. A value with no offset is read as UTC, the convention of `pandas.to_datetime(..., utc=True)` and the only one that does not make a run's bound depend on the timezone of the machine executing the notebook. A value that is not an ISO 8601 instant fails the run before the first HTTP call. Normalization applies only to what is sent: the watermark is still stored as the API wrote it, and the max of a batch is still taken over raw record values.
